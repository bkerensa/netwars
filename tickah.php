...
while ($myrow = mysql_fetch_array($result)){
    ...
    if ($c_crystal==1 && $r_imcrystal!=1) {$result2 = mysql_query("UPDATE ".$PA["table"]." SET crystal=crystal+".(25+$crystalroid*6)." WHERE id=$id",$db); dbr(mysql_error());}
    if ($c_crystal==1 && $r_imcrystal==1) {$result2 = mysql_query("UPDATE ".$PA["table"]." SET crystal=crystal+".(45+$crystalroid*6)." WHERE id=$id",$db); dbr(mysql_error());}

    if ($c_metal==1 && $r_immetal!=1) {$result2 = mysql_query("UPDATE ".$PA["table"]." SET metal=metal+".(15+$metalroid*6)." WHERE id=$id",$db); dbr(mysql_error());}
    if ($c_metal==1 && $r_immetal==1) {$result2 = mysql_query("UPDATE ".$PA["table"]." SET metal=metal+".(33+$metalroid*6)." WHERE id=$id",$db); dbr(mysql_error());}
